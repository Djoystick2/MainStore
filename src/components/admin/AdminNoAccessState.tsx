...
function getNoSessionCopy(input: {
  ...
  if (input.hasInitData && (input.status === 'pending' || input.status === 'ready')) {
    return {
      ...
      description:
        'Админка откроется автоматически, как только подтвердится сессия магазина в Telegram.',
  ...
  if (input.isTelegramRuntime && input.status === 'idle') {
    return {
      ...
      description:
        'Ждём данные запуска от Telegram. Если экран не обновится автоматически, перезапустите магазин из бота.',
      actionLabel: 'Обновить экран',
    };
  }
  ...
  if (input.isTelegramRuntime && input.status === 'failed') {
    const description =
      input.error === 'init_data_unavailable'
        ...
        : input.error
          ? `Магазин открыт из Telegram, но серверная сессия не создалась. Код: ${input.error}.`
